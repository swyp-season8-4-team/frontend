...
import type { CommunityCategory, CommunityDessertReviewCategory } from '@repo/entity/src/community';
...
export default function ReviewWriteForm({ initialReview }: Props) {
  ...
  const [selectedCategory, setSelectedCategory] = useState<CommunityDessertReviewCategory | null>(initialReview?.category ?? null);
  ...
  useEffect(() => {
    if (initialReview?.contents && editorRef.current) {
      ...
      initialReview.contents.forEach(content => {
        if (content.type === 'image' && content.imageId !== undefined && content.imageId > maxImageId) {
  ...
  const isFormValid = useMemo(() => {
    const editorHasContent = editorRef.current && 
    ...
    const walkNodes = (node: Node) => {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (element.classList.contains('image-container')) {
          ...
          if (img) {
            const imageId = img.dataset.imageId ? parseInt(img.dataset.imageId) : undefined;
    ...
    editorRef.current.childNodes.forEach(node => {
      ...
      if (result) {
        if (Array.isArray(result)) {
          ...
          parsedContents.push(result as ReviewContent); // FIXME: 타입 추론 문제 해결
        }
      }
    });
    
    // 이미지 컨테이너 직접 처리 (중첩된 경우를 대비)
    imageContainers.forEach(container => {
      const img = container.querySelector('img');
      if (img) {
        // 이미 처리된 이미지인지 확인
        const imageUrl = img.src;
        const alreadyProcessed = parsedContents.some(
          content => content.type === 'image' && content.imageUrl === imageUrl
        );
    ...
    <form className="flex flex-col gap-[12px] h-full" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col bg-white">
        ...
        <div 
          ref={editorRef}
          contentEditable
          className="w-full px-5 py-4 text-[12px] focus:outline-none min-h-[200px] overflow-y-auto editor-placeholder editor-content relative"
