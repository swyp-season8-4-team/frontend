  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <CarouselContent className="mb-[9px] -ml-0">
          {BANNERS.map((banner, index) => (
            <CarouselItem
              className={cn(
                banner.bgColor,
                // 'relative flex justify-between items-center pt-[13px] pb-[17px] md:pr-[27px] md:py-6 pl-0 rounded-base w-full',
                'relative flex justify-between items-center pt-[13px] pb-[17px] pl-0 rounded-base w-full',
              )}
              key={banner.content}
            >
              <div className="flex items-center">
                ...
                <div className="font-semibold text-[#393939] text-base text-nowrap -tracking-[3%] whitespace-pre-line">
                  {banner.content}
                </div>
              </div>
              <Link
      ...
      <div className="bottom-4 left-1/2 absolute flex gap-2 -translate-x-1/2 transform">
        {BANNERS.map((_, index) => (
          <div
            key={index}
            className={cn(
              // 'w-[3.77px] h-[3.77px] md:w-2 md:h-2 rounded-full',
              'w-[3.77px] h-[3.77px] rounded-full',
