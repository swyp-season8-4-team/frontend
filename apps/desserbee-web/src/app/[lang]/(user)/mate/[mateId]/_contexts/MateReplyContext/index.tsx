...
export const MateReplyContext = createContext<State>(defaultState);

export type FilterMenu = 'latest' | 'register';

interface Props extends WithChildren {
  initialReplyList: MateReply[];
  initialIsLast: boolean;
  initialCounts: number;
}

export const MateReplyProvider = ({ children, initialReplyList, initialIsLast, initialCounts }: Props) => {
  const [selectedFilterMenu, setSelectedFilterMenu] = useState<FilterMenu>('register');
  const [replyList, setReplyList] = useState<MateReply[]>(initialReplyList);
  ...
  return (
    <MateReplyContext.Provider value={{ replyList, isLast, sortLatestReplyList, sortRegisterReplyList }}>
      {children}
    </MateReplyContext.Provider>
  );
};