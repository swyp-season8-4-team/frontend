...
export default function CommunityReviewCard({ review }: Props) {
  ...
  const thumbnailImage = contents.find((content) => content.type === 'image')?.imageUrl;
  ...
  return (
    <div className="flex flex-col items-start gap-[10.704px] rounded-[11.893px] bg-white px-[9px] py-[9.515px]">
      ...
      <div className="flex items-center gap-[9.515px] self-stretch">
        <div className={cn("flex items-center justify-center w-[66.6px] h-[66.6px]", !thumbnailImage ? 'bg-[#D9D9D9]' : '')}>
          <Image
        ...
        <div className="flex flex-col items-start gap-[13.105px] flex-1">
          <div className="flex flex-col items-start gap-[4.914px] self-stretch">
            <span className="line-clamp-1 self-stretch overflow-hidden text-ellipsis text-[#393939] text-[13.105px] font-semibold tracking-[-0.393px]">{title}</span>
            <span className="line-clamp-1 self-stretch overflow-hidden text-ellipsis text-[#393939] text-[11.467px] font-medium tracking-[-0.344px]">{content}</span>
          </div>
          <div className="flex justify-between items-center self-stretch">
            <div className="flex items-center gap-[9.829px]">
              <span className="text-gray-800 text-xs font-medium tracking-tight">{nickname}</span>
              <span className="text-[#9F9F9F] text-[11.467px] font-medium tracking-[-0.344px]">{formatDate(updatedAt)} 조회:{viewCount}</span>
            </div>
            <Link href={`/review/${review.id}`}>
              <Chip
                className={cn("border-none rounded-[75.63px] px-[12.101px] py-[4.538px] text-white bg-[#FFB700]")}
                text={'보러가기'}
              />
            </Link>
          </div>
