import { encrypt } from '@/utils/crypto';
import { OAuthSocialProvider } from '@repo/entity/src/signIn';
import AuthAPIRepository from '@repo/infrastructures/src/repositories/authAPIRepository';
import AuthService from '@repo/usecase/src/authService';
import Link from 'next/link';

const authService = new AuthService({
  authRepository: new AuthAPIRepository(),
});

export default function KakaoSignInButton() {
  const state = encrypt(`${Date.now()}`);

  return (
    <Link
      className="flex w-full items-center justify-start rounded-[6px] bg-[#FEE500] p-4"
      href={authService.getServerSideUrl(OAuthSocialProvider.KAKAO, state)}
    >
      <svg
        width="18"
        height="17"
        viewBox="0 0 18 17"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
      >
        <rect width="18" height="17" fill="url(#pattern0_40000950_100813)" />
        <defs>
          <pattern
            id="pattern0_40000950_100813"
            patternContentUnits="objectBoundingBox"
            width="1"
            height="1"
          >
            <use
              xlinkHref="#image0_40000950_100813"
              transform="scale(0.0277778 0.0294118)"
            />
          </pattern>
          <image
            id="image0_40000950_100813"
            width="36"
            height="34"
            preserveAspectRatio="none"
            xlinkHref="data:image/png;base64,"
          />
        </defs>
      </svg>

      <div className="w-full grow text-center text-lg font-medium">
        카카오 로그인
      </div>
    </Link>
  );
}
